...
interface LinkRelatedNotesDialogProps {
  /** Currently linked note IDs (controlled by parent). */
  selectedIds: string[];
  onChange: (ids: string[]) => void;
  /** The note being edited — excluded from the selectable list. */
  excludeNoteId: string;
  noteAreaUuid?: string;
  noteSubjectUuid?: string;
}

export function LinkRelatedNotesDialog({
  selectedIds,
  onChange,
  excludeNoteId,
  noteAreaUuid = '',
  noteSubjectUuid = '',
}: LinkRelatedNotesDialogProps) {
  ...
  // Sync selectedIds into localSelected when dialog is closed
  useEffect(() => {
    if (!open) {
      setLocalSelected([...selectedIds]);
    }
  ...
  const browsableNotes = useMemo(
    () => notes.filter((n) => n.id !== excludeNoteId),
    [notes, excludeNoteId]
  );
  ...
  const uniqueSubjectsForArea = useMemo(() => {
    ...
    return result.sort((a, b) => a.name.localeCompare(b.name));
  }, [browsableNotes, notesIncluded, filterAreaId]);

  const visibleNotes = useMemo(() => {
    return browsableNotes.filter((note) => {
      const aRel = note.relationships?.field_area?.data;
      const sRel = note.relationships?.field_subject?.data;
      const aId = aRel && !Array.isArray(aRel) ? aRel.id : null;
      const sId = sRel && !Array.isArray(sRel) ? sRel.id : null;
  ...
  }, [browsableNotes, filterAreaId, filterSubjectId]);
  ...
  const knownNoteInfo = useMemo(() => {
    ...
    return map;
  }, [browsableNotes, notesIncluded, searchResults]);
      ...
      <DialogTrigger
        render={
          <Button variant="outline">
            ...
            {selectedIds.length > 0 ? 'Notes' : 'Link Notes'}
